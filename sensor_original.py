...
CSV_DIR = HERE / "config" /  "csv_data_2"
CSV_DIR.mkdir(exist_ok=True)

def measure(*, samples_rate: int = 1000, averages_rate: int = 100, config_name:str = "config.toml", stop_measurement=None, count=None):
    with open(HERE / config_name, mode="rb") as f:
        config = tomli.load(f)
